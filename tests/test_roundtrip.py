...
either domain. This test verifies the transform is lossless.
"""

import numpy as np

from soc import Shape
from soc.generators import curve_to_coeffs, gen_circle, gen_polygon, gen_star
from soc.core.constants import N, M
...
def test_coeffs_are_bandlimited_to_N():
    ...
    # A curve with high-frequency content beyond N
    t = np.linspace(0, 1, M, endpoint=False)
    z = np.exp(1j * 2 * np.pi * 20 * t)  # k=20, far beyond N

    shape = curve_to_coeffs(z)

    # All energy should be in the k=20 mode, which is discarded
